...
public abstract class BasicSimWorld implements SimWorld
{
	...
	*/
	...
	/**
	* Returns the floor brightness at the specified co-ordinate.
	*
	...
	*/
	public int getBrightness(double x, double y, double z)
	{
		...
		for (int i=0;i<objectList.size();i++)
		{
			...
			{
				// see if there is a path at the coordinate
				if (o.getType() == "path")
				{
					return 100;
				}
		...
		// if we've got here then there is no path, so return 0
		return 0;
	}
	...
	public boolean hasObstacle(double x, double y, double z)
	{
		...
		for (int i=0;i<objectList.size();i++)
		{
			...
			if(s.contains(x,z) && !(o instanceof SimSensor))
			{
				...
				if (o.getType() == "path")
				{
					//paths are not considered as obstacles
					return false;
				}
				else
					return true;
	...
	*/
	...
	{
		if (o.getType() == "path" || p.getType() == "path")
		{
			//can't collide with paths
			return false;
		}
		else
		{
